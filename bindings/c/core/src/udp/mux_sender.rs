...
//! not a `TST_E_CLOSED`. `tst_udp_mux_sender_cancel` (ABI 0.22) is the
//! NON-FREEING cross-thread cancel: it fires the same `UdpCancelHandle`
//! without taking the slot and leaves the handle valid for its owner to
//! `_close`.
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn tst_udp_mux_sender_cancel(p: *mut TstUdpMuxSender) -> libc::c_int {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn tst_udp_mux_sender_finish(p: *mut TstUdpMuxSender) -> libc::c_int {
