...
//! FREES the handle: a push already in flight completes normally (the close
//! waits for the slot in `take()`), and there is no such thing as a push
//! after the close — that would be a use-after-free, not a `TST_E_CLOSED`.
//! `tst_rist_mux_sender_cancel` (ABI 0.22) is the NON-FREEING cross-thread
//! cancel: it fires the same `RistCancelHandle` without taking the slot and
//! leaves the handle valid for its owner to `_close`.
//!
...
pub struct TstRistMuxSender {
    pub(crate) inner: CHandle<MuxSender<RistTransport>>,
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn tst_rist_mux_sender_cancel(p: *mut TstRistMuxSender) -> libc::c_int {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn tst_rist_mux_sender_finish(p: *mut TstRistMuxSender) -> libc::c_int {
