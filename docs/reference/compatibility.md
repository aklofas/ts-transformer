...
| `RecvTransport` trait | ✅ Full | Receive-side counterpart to `Transport`: `recv_bytes`, `max_payload`, `is_alive`, `close`, plus the defaulted `cancel_handle`. Implemented by `SrtTransport`, `TcpTransport`, `RtpRecvTransport`, `UdpRecvTransport`, `RistRecvTransport`, the `ManagedRecvTransport<R>` decorator, and any consumer-side mock. `cancel_handle` is overridden — on the send trait by `SrtTransport` / `RtpTransport` / `TcpTransport` and on this receive trait by `SrtTransport` / `RtpRecvTransport` / `TcpTransport`, so shells and the managed wrappers see it either way; `ManagedRecvTransport<R>` overrides it unconditionally (its own cancel latch works even around an uncancellable inner); UDP and RIST keep the `None` default (cooperative shutdown only). See [`srt-cancel-handle.md`](/docs/reference/srt-cancel-handle.md). |
...
| **MISB ST 1204.3** | Motion Imagery Identification System (MIIS) Core Identifier | ✅ `klv::st1204::{decode, encode_to_vec}` + ST 0601 Tag 94 (`miis_core_id`). Rust, Python, and JVM (the C ABI types ST 0601 only — the `tst_st0601_*` family, ABI 0.21 — so this set is Rust / Python / JVM). See [`stanag-4609.md`](/docs/reference/stanag-4609.md). |
...
| `tst-c` | ✅ Full | cdylib + staticlib + cbindgen-generated `tstrans.h` + pkg-config. ABI version **0.21** (additive minor bumps). Multi-platform Tier 1 (Linux x86_64 + aarch64 + macOS arm64 + Windows MSVC all gating). |
